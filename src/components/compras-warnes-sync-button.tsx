...
export function ComprasWarnesSyncButton() {
  ...
  async function readRequest(id: number) {
    const response = await fetch(
      `/api/compras/sync-warnes?id=${encodeURIComponent(String(id))}`,
      {
        method: "GET",
        cache: "no-store",
  ...
  async function handleClick() {
    ...
    try {
      const response = await fetch("/api/compras/sync-warnes", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !body.ok || !body.request) {
        throw new Error(
          body.error || "No se pudo iniciar la actualización.",
        );
      }
      ...
      setStatus("ERROR");
      setMessage(
        cause instanceof Error ? cause.message : "Error al actualizar",
      );
    }
  }
