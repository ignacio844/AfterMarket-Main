...
async function main() {
  ...
  const report = { batchId, skuCount: rows.length, needsReview: conflictSkus };
  if (!process.argv.includes("--apply")) {
    process.stdout.write(`${JSON.stringify({ ...report, dryRun: true }, null, 2)}\n`);
    return;
  }
  const { data: imported, error: importError } = await db.rpc("compras_gestion_importar", { p_registros: rows, p_lote_origen: batchId });
  if (importError) throw new Error(importError.message);
  if (Number(imported) !== rows.length) throw new Error("La cantidad importada no coincide.");
